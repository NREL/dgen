...
DROP TABLE IF EXISTS diffusion_wind_data.dsire_incentives_lookup_ind;
CREATE TABLE diffusion_wind_data.dsire_incentives_lookup_ind AS
with a as 
(
...
ON diffusion_wind_data.dsire_incentives_lookup_ind 
using btree(pt_gid);
...
FROM diffusion_wind_data.dsire_incentives_lookup_ind
group by pt_gid;
...
------------------------------------------------------------------------------------------------------------
--create the lookup table		
DROP TABLE IF EXISTS diffusion_solar_data.dsire_incentives_lookup_ind;
CREATE TABLE diffusion_solar_data.dsire_incentives_lookup_ind AS
with a as 
(
...
CREATE INDEX dsire_incentives_lookup_ind_pt_gid_btree 
ON diffusion_solar_data.dsire_incentives_lookup_ind 
using btree(pt_gid);
...
FROM diffusion_solar_data.dsire_incentives_lookup_ind
group by pt_gid;
...
ALTER TABLE diffusion_shared.pt_grid_us_ind_new set (fillfactor = 50);
...
from diffusion_wind_data.pt_grid_us_ind_new_census_2010_block_lkup b
where a.gid = b.gid;
...
select count(*)
FROM diffusion_shared.pt_grid_us_ind_new
--  where acres_per_hu < 3 -- 438033
where acres_per_hu < 1 -- 323661
where acres_per_hu < .5 -- 228893
...
-- add an index
CREATE INDEX pt_grid_us_ind_new_acres_per_hu_btree
ON diffusion_shared.pt_grid_us_ind_new
...
SELECT parsel_2('dav-gis','mgleason','mgleason',
		'diffusion_shared.pt_grid_us_ind',
		'gid',

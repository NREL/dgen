...
DROP TABLE IF EXISTS diffusion_wind_data.missing_ij_icf_com_points;
CREATE TABLE diffusion_wind_data.missing_ij_icf_com_points AS
...
CREATE INDEX missing_ij_icf_com_points_the_geom_96703_gist 
ON diffusion_wind_data.missing_ij_icf_com_points 
using gist(the_geom_96703);

-- find which points can be fixed by going up or down one cfbin in the same i j cell
DROP TABLE IF EXISTS diffusion_wind_data.com_points_adjusted_1cfbin;
CREATE TABLE diffusion_wind_data.com_points_adjusted_1cfbin AS
with same_cell as (
	SELECT a.*, 
	...
	FROM diffusion_wind_data.missing_ij_icf_com_points a
...
-- 641 rows (95% of the points)


DROP TABLE IF EXISTS diffusion_wind_data.com_points_adjusted_ij;
CREATE TABLE diffusion_wind_data.com_points_adjusted_ij AS
with unfixed as 
(
	SELECT a.gid, a.the_geom_96703, a.i, a.j, a.cf_bin
	FROM diffusion_wind_data.missing_ij_icf_com_points a
	LEFT JOIN diffusion_wind_data.com_points_adjusted_1cfbin b
	ON a.gid = b.gid
	where b.gid is null
),
near_cells As 
...
DROP TABLE IF EXISTS diffusion_wind_data.remaining_missing_com_points;
CREATE TABLE diffusion_wind_data.remaining_missing_com_points AS
SELECT a.*
FROM diffusion_wind_data.missing_ij_icf_com_points a
LEFT JOIN diffusion_wind_data.com_points_adjusted_ij b
ON a.gid = b.gid
lEFT JOIN diffusion_wind_data.com_points_adjusted_1cfbin c
ON a.gid = c.gid
...
-- DROP TABLE IF EXISTS diffusion_wind_data.com_points_adjusted_multi_cfbins;
-- CREATE TABLE diffusion_wind_data.com_points_adjusted_multi_cfbins AS
-- SELECT distinct ON (a.gid) a.gid, a.the_geom_96703, a.i, a.j, a.cf_bin, 
--        b.cf_bin as adjusted_cf_bin
-- FROM diffusion_wind_data.remaining_missing_com_points a
-- LEFT JOIN aws_2014.ij_onshore_cf_bins b
-- ON a.i = b.i
-- and a.j = b.j
...
-- select count(*)
-- FROM diffusion_wind_data.com_points_adjusted_multi_cfbins
-- where adjusted_cf_bin is null;


-- combine all of these, along with non missing points, into a single lookup table
DROP TABLE IF EXISTS diffusion_wind.ij_cfbin_lookup_com_pts_us;
CREATE TABLE diffusion_wind.ij_cfbin_lookup_com_pts_us AS
with notmissing as 
(
...
(
	...
	FROM diffusion_wind_data.com_points_adjusted_1cfbin
),
adjusted_ij AS 
(
	SELECT gid as pt_gid, near_i as i, near_j as j, 
		adjusted_cf_bin::integer/10 as cf_bin, 
		(cf_bin::numeric+15)/(adjusted_cf_bin::numeric+15) as aep_scale_factor
	FROM diffusion_wind_data.com_points_adjusted_ij
)--, 
...
-- 5159001   rows
-- count should match diffusion_wind.pt_grid_us_com
...
-- add primary
ALTER TABLE diffusion_wind.ij_cfbin_lookup_com_pts_us ADD PRIMARY KEY (pt_gid);
-- add foreign key
ALTER TABLE diffusion_wind.ij_cfbin_lookup_com_pts_us
  ADD CONSTRAINT pt_gid_fkey FOREIGN KEY (pt_gid)
      REFERENCES diffusion_shared.pt_grid_us_com (gid) MATCH FULL
      ON UPDATE resTRICT ON DELETE resTRICT;

-- add indices
CREATE INDEX ij_cfbin_lookup_com_pts_us_i_btree ON diffusion_wind.ij_cfbin_lookup_com_pts_us using btree(i);
CREATE INDEX ij_cfbin_lookup_com_pts_us_j_btree ON diffusion_wind.ij_cfbin_lookup_com_pts_us using btree(j);
CREATE INDEX ij_cfbin_lookup_com_pts_us_cf_bin_btree ON diffusion_wind.ij_cfbin_lookup_com_pts_us using btree(cf_bin);

-- check for scale factors of zero
select *
FROM diffusion_wind.ij_cfbin_lookup_com_pts_us
...
where c.height = 50
and c.turbine_id = 1
and b.aep_scale_factor <> 1 -- should return <674 rows

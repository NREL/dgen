...
-- since they don't really have a good place to go, let's put them across the entire state
DROP TABLE IF EXISTS dg_wind.ventyx_ests_2011_res_sales_replace_slivers_w_states;
CREATE TABLE dg_wind.ventyx_ests_2011_res_sales_replace_slivers_w_states AS

SELECT a.gid, a.state_abbr, a.total_residential_sales_mwh, b.the_geom_4326
FROM dg_wind.ests_w_no_pop a
LEFT JOIN ventyx.states_only b
...
and a.state_abbr not in ('AK','HI')
AND a.total_residential_sales_mwh > 0;

-- create a spatial index for this table
...
CREATE TABLE dg_wind.ests_res_no_slivers_pop_sums_us
(gid integer,
pop numeric,
num_tiles integer);

-- run parsel
select parsel_2('dav-gis','dg_wind.ventyx_ests_2011_res_sales_replace_slivers_w_states','gid',
'WITH tile_stats as (
	select a.gid,
		ST_SummaryStats(ST_Clip(b.rast, 1, a.the_geom_4326, true)) as stats
	FROM dg_wind.ventyx_ests_2011_res_sales_replace_slivers_w_states as a
	INNER JOIN dg_wind.ls2012_ntime_res_pop_us b
	ON ST_Intersects(a.the_geom_4326,b.rast)
)
	--aggregate the results from each tile
...
FROM tile_stats
GROUP by gid;'
,'dg_wind.ests_res_no_slivers_pop_sums_us','a',16);

-- check there are no remaining areas with zero pop
SELECT count(*)
FROM dg_wind.ests_res_no_slivers_pop_sums_us
where pop = 0 or pop is null;
...
LEFT JOIN dg_wind.ests_res_no_slivers_pop_sums_us c
ON a.gid = c.gid

...
-- check out which values did not join from the sales table to the polygons
...
------------------------------------------------------------------------------------------------------------------------------------------------------------
-- back fill any remaining service territories with misssing data using state level 2011 values minus ventyx service territory data aggregated by states
------------------------------------------------------------------------------------------------------------------------------------------------------------
-- but first, investigate how this should be done properly:
...
---------
-- investigation 1: compare national level load totals between 2011 ventyx retail sales table, 2011 eia tables 1-4, and backfilled data up to this point (all years
---------
...
-- so this is the problem: the 2011 ventyx sales totals (from ventyx.electric_utility_rates_2011) are essentially the same at the national level as the 2011 eia tables 1 - 4
	-- this suggests that ventyx and eia are internally consistent
-- however, the ventyx sales data that actually joins to ventyx service territories is noticeably smaller, even including the 2009 and 2010 backfilling.
	-- this suggests that we can't fully link all of the ventyx sales data to the polygons that they belong to
...
---------
-- investigation 2: compare national level customer totals between 2011 ventyx retail sales table, 2011 eia tables 1-4, and backfilled data up to this point (all years
---------
...
FROM eia.tables_1_thru_4_2011; -- 126,143,072 -- nailed it!

select sum(total_residential_customers)
FROM dg_wind.electric_service_territories_states_with_rates; -- 124,676,004

-- findings are consistent with those for load, above
...
FROM eia.tables_1_thru_4_2011; -- 17,638,062 -- close enough

select sum(total_commercial_customers)
FROM dg_wind.electric_service_territories_states_with_rates; -- 17,338,409
-- same findings as for load
...
FROM eia.tables_1_thru_4_2011; -- 727,920 -- close enough

select sum(total_industrial_customers)
FROM dg_wind.electric_service_territories_states_with_rates; --730,106-- this is a problem *******
-- where data_year = 2011; -- 723,260


---------
-- investigation 3: compare national level revenue totals between 2011 ventyx retail sales table, 2011 eia tables 1-4, and backfilled data up to this point (all years
---------
...
FROM dg_wind.electric_service_territories_states_with_rates; -- 159,398,200.8
...
FROM eia.tables_1_thru_4_2011; -- 135,926,483.9 -- close enough

select sum(total_commercial_revenue_000s)
FROM dg_wind.electric_service_territories_states_with_rates; -- 112,738,535.4
-- same findings as for load
...
FROM eia.tables_1_thru_4_2011; -- 67,605,747.7 -- close enough

select sum(total_industrial_revenue_000s)
FROM dg_wind.electric_service_territories_states_with_rates; --55,191,413.5
-- where data_year = 2011; -- 723,260

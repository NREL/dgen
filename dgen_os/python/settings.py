...
class ModelSettings(object):
    ...
    def __init__(self):
        ...
        self.output_reeds_data = None  # bool
    ...
    def add_config(self, config):
        ...
        self.set('output_reeds_data', config.output_reeds_data)
    ...
    def validate_property(self, property_name):
        ...
        if property_name == 'model_init':
            ...
            if self.sample_pct > 1:
                raise ValueError(
                    'Invalid {}: value must be <= 0'.format(property_name))
            if self.sample_pct > 0.05:
                warnings.warn(
                    'High {}: using values > 0.05 may result in very slow model run times'.format(property_name))
        ...
        elif property_name == 'output_reeds_data':
            # check type
            try:
                check_type(self.get(property_name), bool)
            except TypeError as e:
